...
colors = ["tab:brown","tab:pink","tab:red","tab:blue","tab:green","tab:purple","tab:orange","tab:olive"]
...
for i in range(0,len(zis)):
    ...
    for vali in range(4):
        ...
        for yy in range(T.shape[0]):
            for tt in range(48):
#                dT[yy,tt] = T[yy,3*24*2+tt]-np.mean(T[yy,3*24*2:4*24*2])
                dT[yy,tt] = 0.5*(T[yy,3*24*2-6*2+tt]+T[yy,4*24*2-6*2+tt])-np.mean(T[yy,3*24*2-6*2:-6*2])
        ...
        if vali+plot_n*4 in [0,4,8]:
            axes[-1].set_ylabel("Model level "+str(zi)+"\n ~"+str(zi_h)+" meters above surface",fontsize=14)
...
#            segi_lat, segi_lon = wrf.xy_to_ll(ncfile, segi_y, segi_x)
            ...
            plt.scatter(4.75,dT.shape[0]-segs[vali,jj],marker="x",s=45,color=np.flip(colors_crosses)[c])
            c+=1
...
legend_elements = [Line2D([0], [0], color=colors[0], lw=2, label="00"),
                    Line2D([0], [0], color=colors[1], lw=2, label="03"),
                    Line2D([0], [0], color=colors[2], lw=2, label="06"),
                    Line2D([0], [0], color=colors[3], lw=2, label="09"),
                    Line2D([0], [0], color=colors[4], lw=2, label="12"),
                    Line2D([0], [0], color=colors[5], lw=2, label="15"),
                    Line2D([0], [0], color=colors[6], lw=2, label="18"),
                    Line2D([0], [0], color=colors[7], lw=2, label="21")]
...
axes[4].set_title("(e) Gaurishankar",fontsize=14)
axes[8].set_title("(i) Gaurishankar",fontsize=14)

axes[5].set_title("(f) Khumbu",fontsize=14)
axes[9].set_title("(j) Khumbu",fontsize=14)

axes[6].set_title("(g) Makalu",fontsize=14)
axes[10].set_title("(k) Makalu",fontsize=14)

axes[7].set_title("(h) Kanchanjunga",fontsize=14)
axes[11].set_title("(l) Kanchanjunga",fontsize=14)



for j in [8,9,10,11]:
    axes[j].set_xlabel("$\\theta-\\overline{\\theta}$ [K]",fontsize=14)
    # for i in range(0,8):
